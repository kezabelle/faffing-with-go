...
var pbkdf2s = []PDBKDF2{
	...
	{"Django 1.11 PBKDF2 (SHA256)", "pbkdf2_sha256$36000$woof$gOvAuiZY5KyuxBOyLOs5+tovjqb6wEHQsWM0mvRxmNo=", DJ_111_PBKDF2},
	{"Django 1.11 PBKDF2 (SHA1)", "pbkdf2_sha1$36000$woof$zcBBwbvZBLdlZyu1uHNmrzlnya4=", DJ_111_PBKDF2_SHA1},
}
...
func TestPBKDF2PasswordHasher_Encode(t *testing.T) {
	for _, hashable := range pbkdf2s {
		t.Run(hashable.name, func(t *testing.T) {
			encoded, _ := hashable.hasher.Encode("test", "woof", hashable.hasher.iterations)
			expected := hashable.encoded_value
			if encoded != expected {
				t.Errorf("Expected %s, got %s", expected, encoded)
			}
